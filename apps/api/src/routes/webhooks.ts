...
async function handleWhatsAppWebhook(
  ...
  if (!evolutionWebhookAuthorized(env, request)) {
    ...
    await logWhatsAppWebhookHit(request.body, "auth_rejected");
    return reply.status(401).send({ error: "Unauthorized" });
  }

  await logWhatsAppWebhookHit(request.body, "auth_ok");
  ...
  if (!data) {
    ...
    await logWhatsAppWebhookHit(request.body, "parse_failed");
    return reply.send({ ok: true, skipped: true, reason: "parse_failed" });
  }

  if (!data.key || data.key.fromMe) {
    await logWhatsAppWebhookHit(request.body, "skipped_from_me");
  ...
  if (!contact) {
    ...
    await logWhatsAppWebhookHit(request.body, "invalid_sender_jid");
  ...
  if (!contact.storedPhone) {
    ...
    await logWhatsAppWebhookHit(request.body, "no_stored_phone", replyTo);
    try {
  ...
  if (!text.trim() && messageType === "text") {
    ...
    await logWhatsAppWebhookHit(request.body, "empty_text", fromNumber);
  ...
  try {
    ...
    await logWhatsAppWebhookHit(request.body, "process_error", fromNumber);
    try {
      await app.evolution.sendText(
        replyTo,
...
export async function webhookRoutes(app: FastifyInstance): Promise<void> {
  ...
  /** Evolution com webhookByEvents posta em /webhooks/whatsapp/messages-upsert */
